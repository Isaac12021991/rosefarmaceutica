...
class Ion_auth
{
			...
			function info_partner_todo()
	{
        ...
        return $query->row();
				...
				function info_user_todo()
	{

		$usuario=$this->ion_auth_model->user()->row();
		$sql="SELECT * 
		from lu_users as a
		where a.id = '$usuario->id' limit 1";
        $query=$this->db->query($sql);
        return $query->row();
	}

				function co_membresia()
	{
		...
		where a.id = '$usuario->co_partner' and a.in_activo = true and a.in_validado = true limit 1";
        $query=$this->db->query($sql);
        $info_partner = $query->row();
        return $info_partner->co_membresia;
	}
	...
	function conectar_identificar_sesion_usuario($nb_estatus) {
		// Carga las librerias y helpers
		$obj =& get_instance();

		$info_usuario=$this->ion_auth_model->user()->row();
		...
		$data = array(
               'ip' => $obj->input->ip_address(),
               'id_session' => session_id(),
               'co_usuario' => $info_usuario->id,
               'nb_os' => $nb_os,
               'nb_navegador' => $nb_navegador,
               'tx_agente' => $tx_agente,
               'ff_ultima_actividad_time ' => time(),
               'ff_registro_time' => time(),
               'nb_estatus' => $nb_estatus 
            );
		
		// Inserta la accion en la tabla de auditoria
		$obj->db->insert('lu_users_sessions', $data);
	}



	function desconectar_identificar_sesion_usuario($nb_estatus) {
		// Carga las librerias y helpers
		$obj =& get_instance();
		...
		$id_session = session_id();
		// Inserta la accion en la tabla de auditoria
		$obj->db->where('id', $id_session);
		$obj->db->delete('ci_sessions');

		$obj->db->where('co_usuario', $info_usuario->id);
		$obj->db->where('id_session', $id_session);
		$obj->db->delete('lu_users_sessions');
	}
